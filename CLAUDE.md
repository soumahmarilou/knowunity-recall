...
- Every design decision (component choice, value, naming) follows [design-system_1.md](design-system_1.md); every value comes from [tokens_1.json](tokens_1.json).
...
- Never invent a token value not in `tokens_1.json` — see design-system_1.md § "Never do this" for the full list (no reading primitives directly, no `accent/1-4` for status/tier, no duplicating a component's layers).
...
- [tokens_1.json](tokens_1.json) — actual color/spacing/type/radius values; read whenever a design value is needed.
